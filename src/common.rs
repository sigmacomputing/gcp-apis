...
pub trait Delegate {
    ...
    fn response_json_decode_error(&mut self,
...
impl<'a> Read for MultiPartReader<'a> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        match (self.raw_parts.len(),
               ...
               self.last_part_boundary.is_none()) {
            (_, _, false) => {
                let br = self.last_part_boundary
                    .as_mut()
                    .unwrap()
                    .read(buf)
                    .unwrap_or(0);
            ...
            Ok(bytes_read) => {
                if hb < buf.len() && bytes_read == 0 {
                    if self.is_last_part() {
                        ...
                        self.last_part_boundary =
                            Some(Cursor::new(format!("{}--{}--", LINE_ENDING, BOUNDARY)
                                                 .into_bytes()))
                    }
    ...
    where A: oauth2::GetToken
{
    ...
    pub fn upload(&mut self) -> Option<hyper::Result<hyper::client::Response>> {
        let mut start = match self.start_at {
            Some(s) => s,
            None => {
                match self.query_transfer_status() {
                    Ok(s) => s,
            ...
            match res {
                Ok(mut res) => {
                    ...
                    if !res.status.is_success() {
                        ...
                        if let Retry::After(d) = self.delegate
                               .http_failure(&res,
